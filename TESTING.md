...
- `job1` переходит в `SUCCEEDED`, как только `AssignmentStatus` оркестратора дойдёт до `Complete` —
  **это не значит, что робот реально доделал работу**, только что оркестратор поставил проект в
  очередь выполнения (см. раздел 2b и риски в `roadmap.md`). Поэтому `checkInput`/`checkOutput`
  здесь не опциональны для проверки "точно ли что-то произошло" — без них "успех" сценария ничего
  не доказывает, кроме того, что запрос на запуск был принят.
...
`checkInput` и `checkOutput` в основном сценарии (раздел 2) — это и есть автоматическая проверка
результата (`QUEUE_CHECK`): каждый ждёт, пока фактическое распределение элементов очереди по
статусу не совпадёт с тем, что задал автор сценария, и явно проваливает шаг (с деталями
...
1. `queueIn` → `RUNNING` → `SUCCEEDED` (очередь и транзакция созданы), у него появляется
   `orchestratorQueueId`.
...
3. `job1` → `RUNNING`, у него появляется `orchestratorAssignmentId`; проект читает `queueIn` и
   пишет в `queueOut` изнутри — мы это не вызываем явно.
4. `job1` → `SUCCEEDED` — **это означает только, что оркестратор поставил Assignment в очередь
   выполнения** (`AssignmentStatus.Complete`), не то, что робот реально доделал работу. Дальше
   движок сразу идёт к `checkInput`, не дожидаясь ничего дополнительно.
5. `checkInput` → `RUNNING` (может занять время — он **поллит** `queueIn`, пока `tx-1` не дойдёт
   до `Success`/`Error`/`BusinessError`, либо не истечёт `timeoutSeconds`) → `SUCCEEDED`, когда
   реальный статус транзакции совпал с `expectedStatusCounts`. Это и есть настоящее подтверждение,
   что робот обработал входные данные — не шаг 4.
...
- [ ] `checkInput` (после `job1`) дожидается реального терминального статуса `tx-1` во входной
      очереди — не путать с `job1.SUCCEEDED`, который наступает раньше и ничего не доказывает
- [ ] `checkOutput` находит записанные проектом транзакции в выходной очереди по префиксу `tx-1`
