...
- `QUEUE_CHECK` (`QueueCheckStepExecutor`) — образец шага, который не создаёт ничего в
  оркестраторе, а поллит существующее состояние до совпадения с ожиданием или таймаута. Полезный
  паттерн для любой будущей "проверочной" логики: не встраивать проверку внутрь шага, который
  создаёт сущность (`JobStepExecutor`/`QueueStepExecutor`), а выносить в отдельный шаг сразу после
  него в DAG — так проверка переиспользуема и не завязана на то, что именно её вызвало.
